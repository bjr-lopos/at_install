...
- Before changing any scenario cadence/interval/retry behavior: estimate the change in **tags-transmissions-per-tag-per-hour** and call it out. Observed anomaly (2026-06-08): tags fix ~every 37 s despite a 95 s `plan.interval` — i.e. TDoA is effectively ~3× over-scheduled vs the plan, which is a **battery** concern, not just airtime. If you "improve" fix rate, you are likely spending battery; say so.
